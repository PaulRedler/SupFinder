...
/**
 * Migration to format ecole logoUrl with proper casing and spaces
 */
final class Version20260323000007 extends AbstractMigration
{
    public function getDescription(): string
    {
        return 'Format ecole logoUrl to Title Case with spaces instead of underscores';
    }
    ...
    public function up(Schema $schema): void
    {
        ...
        foreach ($rows as $row) {
            ...
            if (isset($matches[1])) {
                $filename = $matches[1];  // e.g., 'universite_d_avignon'
                ...
                // Join with spaces and create new path
                $newFilename = implode(' ', $titleCaseWords);  // e.g., 'Universite D Avignon'
    ...
    {
        // Rollback: convert spaces back to underscores and lowercase
        $connection = $this->connection;
        $rows = $connection->fetchAllAssociative('SELECT id, logo_url FROM ecole WHERE logo_url IS NOT NULL');
        ...
        foreach ($rows as $row) {
            ...
            // Parse: 'images/Universite D Avignon.png'
            preg_match("/'images\/(.+)\.png'/", $oldPath, $matches);
            
            if (isset($matches[1])) {
                $filename = $matches[1];
                
                // Convert spaces back to underscores and lowercase
